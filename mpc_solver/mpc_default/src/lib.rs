...
pub const MPC_DEFAULT_NUM_DECISION_VARIABLES: usize = 40;

/// Number of parameters
pub const MPC_DEFAULT_NUM_PARAMETERS: usize = 2211;

/// Number of parameters associated with augmented Lagrangian
pub const MPC_DEFAULT_N1: usize = 40;

/// Number of penalty constraints
pub const MPC_DEFAULT_N2: usize = 15;
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(MPC_DEFAULT_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, MPC_DEFAULT_N1, MPC_DEFAULT_N2)
}
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_mpc_default::get_w_cost();
...
pub fn solve(
    ...
    assert_eq!(p.len(), MPC_DEFAULT_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), MPC_DEFAULT_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    // Start by initialising the optimiser interface (e.g., set w=1)
    icasadi_mpc_default::init_mpc_default();
    ...
    if DO_PRECONDITIONING {
        ...
        icasadi_mpc_default::precondition(u, p);

        // Compute initial penalty
        icasadi_mpc_default::initial_penalty(u, p, & mut rho_init);
    }

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_mpc_default::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_mpc_default::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f1 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_mpc_default::mapping_f1(u, p, res);
        Ok(())
    };
    let f2 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_mpc_default::mapping_f2(u, p, res);
        Ok(())
