...
class CBF_LQR_Controller:
    ...
    def compute_control(self, x, x_ref, u_ref, obstacles):
        ...
        try:
            ...
            print(f'Solver did not converge. Status: {solution.exit_status}')
            # print(f'Control input: vel {u_ref[0]}, ang {u_ref[1]}')
...
class ObstacleProcessor:
    def __init__(self,  safety_margin: float = 0.2, max_obstacle_distance: float = 10.0):
        """ 
    ...
    def get_closest_obstacles(self, sensor: LaserScan, robot_pose, num_obstacles: int = 3):
        """ 
        ...
        valid = (~np.isnan(ranges)) & (ranges >= sensor.range_min) & (ranges <= min(sensor.range_max, self.max_obstacle_distance))
